...
class KNNClassifier(object):
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            y_pred[i] = torch.mode(self.y_train[torch.argsort(dist_matrix[:, i])[:self.k]])[0]
            # ========================
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    dists = torch.sqrt(x1_norm + x2_norm.t() - 2 * x1_x2)
    # ========================
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        for fold in range(len(folds)):
            train_indices = torch.cat(folds[: fold] + folds[fold + 1:], dim=0)
            validation_indices = folds[fold]

            dl_train = DataLoader(ds_train, sampler=torch.utils.data.SubsetRandomSampler(train_indices))
            dl_valid = DataLoader(ds_train, sampler=torch.utils.data.SubsetRandomSampler(validation_indices))
            x_valid, y_valid = dataloader_utils.flatten(dl_valid)

            model.train(dl_train)
            y_pred = model.predict(x_valid)

            k_fold_accuracies.append(accuracy(y_pred, y_valid))
        accuracies.append(k_fold_accuracies)
        # ========================
