...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        w_opt = None
        # ====== YOUR CODE: ======
        reg = self.reg_lambda * y.shape[0] * np.eye(X.shape[1])
        reg[0][0] = 0
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    # ====== YOUR CODE: ======
    corr = df.corrwith(df[target_feature]).abs().sort_values(ascending=False)[1:n+1]
    top_n_features, top_n_corr = corr.index, corr.values
    # ========================
