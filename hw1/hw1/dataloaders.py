...
class FirstLastSampler(Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        for i in range(size // 2):
            yield indices[i]
            yield indices[(size - 1) - i]
        if size % 2 == 1:
            yield indices[size // 2]
        # ========================
...
def create_train_validation_loaders(
    ...
    train_size = math.floor(dataset_size * (1 - validation_ratio))
    ...
    dl_train = torch.utils.data.DataLoader(dataset, sampler=train_sampler,
                                           batch_size=batch_size, num_workers=num_workers)
    dl_valid = torch.utils.data.DataLoader(dataset, sampler=valid_sampler,
                                           batch_size=batch_size, num_workers=num_workers)
    # ========================
